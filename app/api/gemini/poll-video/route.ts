...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('Polling Vertex AI operation:', operationId)
      ...
      if (response.ok) {
        ...
        console.log('Poll response status:', data.done ? 'done' : 'processing')
        console.log('Poll response data:', JSON.stringify(data, null, 2).substring(0, 500))
          ...
          if (videoUrlToDownload) {
            ...
            if (!videoResponse.ok) {
              ...
              if (!videoResponse2.ok) {
                throw new Error(`Failed to download video: ${videoResponse2.statusText}`)
              }
            ...
            console.log('Video operation completed but no video URI found in response')
        ...
        console.log('Poll API error:', response.status)
        console.log('Poll API error response:', errorText.substring(0, 500))
        
        // Try alternative URL format if we get 404
        if (response.status === 404 && operationId.includes('/operations/')) {
          ...
          try {
            ...
            if (altResponse.ok) {
              ...
              console.log('Alternative poll successful!')
              // Process the response the same way
              if (altData.done && altData.response) {
                const videoUrlToDownload = altData.response.videoUrl || altData.response.video || altData.response.videoUri
                if (videoUrlToDownload) {
                  console.log('Video ready! Downloading from:', videoUrlToDownload)
                  const videoResponse = await fetch(videoUrlToDownload)
                  if (!videoResponse.ok) {
              ...
              if (altData.done && altData.error) {
                console.error('Operation failed:', altData.error)
                if (fs.existsSync(operationFile)) {
                  fs.unlinkSync(operationFile)
                }
                return NextResponse.json({
                  status: 'error',
                  error: altData.error.message || 'Video generation failed',
                  details: altData.error,
                }, { status: 500 })
              }
              return NextResponse.json({
                status: 'processing',
                message: 'Video generation in progress...',
                progress: altData.metadata?.progress || null,
              })
            }
          } catch (altError) {
            console.log('Alternative polling also failed:', altError)
          }
        }
      }
    } catch (pollError) {
    ...
    if (fs.existsSync(operationFile)) {
      ...
      if (operationData.operationId === operationId) {
        return NextResponse.json({
          status: 'processing',
          message: 'Video generation in progress. Please check again in a few minutes.',
        })
      }
    }
